...
const EditTvshow = () => {
    ...
    useEffect(() => {
        const fetchMovieData = async () => {
            try {
                const response = await axios.get(`http://127.0.0.1:5002/api/getTvshows/${movieId}`);
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            const backendResponse = await axios.put(`http://127.0.0.1:5002/api/tvshowEdit/${movieId}`, formData);
    ...
    return (
        <div>
            <div>
                <h1 className='text-3xl font-bold text-center my-8'>Edit TVshow Data</h1>
            </div>
